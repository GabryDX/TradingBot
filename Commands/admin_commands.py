...
comandi_admin = ["/admin", "/addadmin", "/aggiungititolo", "/getadmins",
...
async def admin_command(messaggio, bot):
    ...
    if contain:
        if admin.is_admin(messaggio.from_user.username):
            ...
            elif testo.startswith("/aggiungititolo"):
                await aggiungi_titolo(messaggio)
...
async def aggiungi_titolo(messaggio):
    testo = messaggio.text
    if "[" in testo and "]" in testo:
        rimuovi = "/aggiungititolo ["
        s = testo.replace(rimuovi, "")
        index = utenti.find_str(s, "]")
        key = s[:index].replace("]", "").strip()
        value = s.replace(key, "").replace("]", "").strip()
        textfiles.append(key + "___" + value + "\n", azioni.AZIONI)
        await messaggio.reply_text("*Azione " + key + " inserita*", parse_mode=MARKDOWN)
